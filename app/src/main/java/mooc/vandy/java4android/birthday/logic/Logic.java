...
public class Logic
    ...
    public double calculate(int size, int count) {
        ...
        for(int i = 1; i <= count; ++i){
            set.clear();
            success+=findSameBirthday(new Random(i), set, size);
        }
        // TODO -- add your code here
        return (success/count)*100;
    }

    public int randomGenerator(Random generator) {
        return generator.nextInt(365);
    }
    public int findSameBirthday(Random generator, HashSet<Integer> set, int size){
        int j = 0;
        while(j < size){
            int num = randomGenerator(generator);
            if(!set.contains(num)){
                set.add(num);
            }
            else{
